...
// ===== PENERAPAN POINTER =====
void tambahStokPointer(RobuxItem *data, int index, int tambah) {
    if (index >= 0) {
        data[index].stok += tambah;
        cout << "Stok berhasil ditambahkan! Stok sekarang: " << data[index].stok << "\n";
    } else {
        cout << "Index tidak valid!\n";
    }
}

void tampilkanDetailItem(RobuxItem *item) {
    cout << "\n===== Detail Item Robux =====\n";
    cout << "Nama  : " << item->nama << "\n";
    cout << "Harga : " << item->harga << "\n";
    cout << "Stok  : " << item->stok << "\n";
    cout << "==============================\n";
}

// ===== SORTING =====
void sortNamaDescending(RobuxItem data[], int total) {
...
void sortHargaAscending(RobuxItem data[], int total) {
    for (int i = 0; i < total - 1; i++) {
        for (int j = 0; j < total - i - 1; j++) {
    ...
    cout << "Data berhasil diurutkan berdasarkan harga (murah ke mahal).\n";
}

void sortStokDescending(RobuxItem data[], int total) {
    for (int i = 0; i < total - 1; i++) {
        for (int j = 0; j < total - i - 1; j++) {
    ...
    cout << "Data berhasil diurutkan berdasarkan stok (terbanyak).\n";
}
    ...
    while (true) {
        ...
        if (isAdmin) {
            do {
                ...
                cout << "3. Edit Robux\n";
                cout << "4. Hapus Robux\n";
                cout << "5. Tambah Stok Robux (Pointer)\n";
                cout << "6. Tampilkan Detail Item (Pointer)\n";
                cout << "7. Logout dan Kembali ke Login\n";
                cout << "8. Keluar dari Program\n";
                cout << "9. Sorting Robux\n";
                cout << "Pilihan Anda: ";
                cin >> pilihan;
                ...
                switch (pilihan) {
                    ...
                    case 5: {
                        int index, tambah;
                        cout << "Masukkan nomor item untuk tambah stok: ";
                        cin >> index;
                        if (index > 0 && index <= totalRobux) {
                            cout << "Jumlah stok yang ingin ditambah: ";
                            cin >> tambah;
                            tambahStokPointer(&robux[index - 1], index - 1, tambah);
                        } else {
                            cout << "Nomor tidak valid!\n";
                        }
                        break;
                    ...
                    case 6: {
                        int index;
                        cout << "Masukkan nomor item untuk ditampilkan: ";
                        cin >> index;
                        if (index > 0 && index <= totalRobux) {
                            tampilkanDetailItem(&robux[index - 1]);
                        } else {
                            cout << "Nomor tidak valid!\n";
                        }
                        break;
                    }
                    case 7: cout << "Logout... Kembali ke halaman login.\n"; break;
                    case 8: cout << "Keluar dari program...\n"; return 0;
                    case 9: {
                        int pilihSort;
                        cout << "\n=== Pilihan Sorting ===\n";
                        cout << "1. Sort Nama (Z-A)\n";
                        cout << "2. Sort Harga (Murah ke Mahal)\n";
                        cout << "3. Sort Stok (Terbanyak ke Terkecil)\n";
                        cout << "Pilihan Anda: ";
                        cin >> pilihSort;
                        switch (pilihSort) {
                            case 1: sortNamaDescending(robux, totalRobux); break;
                            case 2: sortHargaAscending(robux, totalRobux); break;
                            case 3: sortStokDescending(robux, totalRobux); break;
